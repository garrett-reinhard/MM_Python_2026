...
class Reaction:
    ...
    def __init__(self):
        ...
        self.stir_speed = 0
        """Stir Speed this reaction took place at"""
...
class Experiment:
    ...
    def __init__ (self, devices):
        ...
        self.shim_source = "chloroform" #TODO pull from startup script
    ...
    def set_vial(self, requested_temperature,requested_stir_speed, reaction: Reaction):
        """Send commands to the device driver to allocate a vial for this reaction+key"""
        set_vial_command = "_SET_EXPERIMENT_VIAL"
        heater_file = self.experiment_folder + "\heater.txt"
        set_vial_args = [self.next_vial, requested_temperature, requested_stir_speed, heater_file] #next vial name, temperature request, reaction save folder (for reading return temp.)
        ...
        if not self.DEBUG:
            with open(heater_file) as file:
                    heater_settings = file.read().splitlines()
                    reaction.temperature = float(heater_settings[0])
                    reaction.hotplate = heater_settings[1]
                    reaction.stir_speed = heater_settings[2]

                    reaction.save_folder = reaction.save_folder+f"_{reaction.hotplate}"
                    os.makedirs(reaction.save_folder, exist_ok=True)
        ...
        while not reaction.reaction_finished: 
            ...
            while not good_scan:
                ...
                if scan_linewidth < self.ACCEPTABLE_SHIM_LINEWIDTH:
                    print(f"BAD SCAN: Linewidth too large ({scan_linewidth}); attempting shimming. . .")
                    self.shim_experiment()
                    bad_scan_count = 0
                    good_scan = False
                #PLOT: NMR scan
       ...
       Uses self.reactions[current_reaction] as primary source of data, treats entry  -1 as yet to be processed data"""
        ...
        if predicted_times != []:
            ...
            if self.current_runtime+self.MINIMUM_TIME > average_prediction:
                ...
                self.EDBO_add_data(reaction.final_reaction_time,reaction.product_yield)
                return 0
    ...
    def start_experiment(self):
        ...
        #Begin Reaction looping
        print(self.number_of_reactions)
        for reaction_number in range(int(self.number_of_reactions)):
            ...
            self.next_vial = self.experiment_name+f"reaction_{reaction_number}"
            ...
            self.set_vial(self.next_temperature,self.next_stir_speed, current_reaction ) #TODO get temperature return, pass to current_reaction.temperature
    ...
    def EDBO_reaction_setup(self):
        ...
        self.reaction_scope =EDBOplus().run(
            filename=f'{self.experiment_name}.csv',  # Previously generated scope.
        ...
        self.next_temperature = self.reaction_scope.loc[0, 'temperature']
        self.next_stir_speed = self.reaction_scope.loc[0, 'stir_speed']
        self.next_solvent = self.reaction_scope.loc[0, 'solvent'] 
        self.next_reagent = self.reaction_scope.loc[0, 'reagent'] 
        self.next_solvent_volume = self.reaction_scope.loc[0, 'solvent_volume']
        self.next_reagent_volume = self.reaction_scope.loc[0, 'reagent_volume']
