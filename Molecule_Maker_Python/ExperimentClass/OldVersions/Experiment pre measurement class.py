...
class Experiment:
    ...
    def set_vial(self):
        ...
        set_vial_args = [self.next_vial]
        print(self.next_vial)

        self.key.release()
        self.Devices.issue_command(set_vial_command, set_vial_args, self.key)
        self.key.acquire()
    ...
    def hold_until_complete(self):
        ...
        self.reactions[self.current_reaction]["data_processing"] = {}
        ...
        while not self.reaction_finished:
            ...
            empty_dict = {}
            self.reactions[self.current_reaction]["measurements"].append(empty_dict) 
            self.reactions[self.current_reaction]["measurements"][-1]["file"] = scan_filepath
            self.reactions[self.current_reaction]["measurements"][-1]["name"] = scan_name
            self.reactions[self.current_reaction]["measurements"][-1]["data"] = scan_data #Has intensity and PPM catagories
            self.reactions[self.current_reaction]["measurements"][-1]["misc_data"] = scan_dictionary
    ...
    def start_experiment(self):
        ...
        for reaction in range(int(self.info["reactions"])):
            self.reactions.append({})
            self.current_reaction = reaction #Declaring this prevents needing to pass args to the data processing functions.  Makes the flow easier to read
            self.next_vial = self.info["experiment_name"]+f"reaction_{reaction}"
...
class Measurement:
    ...
    def __init__(self):
        self.scan_filepath = ""
        self.scan_name = ""
        self.scan_data = None #Has intensity and PPM catagories
