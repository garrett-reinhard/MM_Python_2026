...
class NodeTree:
    ...
    def set_experiment_vial(self, name, requested_temperature=None):
        ...
        if requested_temperature != None:
            ...
            for component in self.components:
                ...
                if self.components[component]["type"] == "vial":
                    if self.components[component]["used"] == False:
                        ...
                        if self.components[component]["hotplate_temp"] == False and acceptable_vial == False:
                            unused_vial = str(component)

                        #Condition: Unused vial with acceptable temperature already set
                        elif (self.components[component]["hotplate_temp"] != False) and (lower_temp <= self.components[component]["hotplate_temp"]) and (upper_temp >= self.components[component]["hotplate_temp"]):
                            acceptable_vial = True
                            unused_vial = component
            ...
            if acceptable_vial:
                self.components[name] = self.components[unused_vial] 
                del self.components[component]
                self.components[name]["name"] = name
                self.components[name]["used"] = True

            elif unused_vial != None:
                self.components[name] = self.components[unused_vial] 
                del self.components[component]
                self.components[name]["name"] = name
                self.components[name]["used"] = True

                self.components[name]["hoteplate_temp"] = requested_temperature
        ...
        #Condition
            ...
            for component in self.components:
                ...
                if self.components[component]["type"] == "vial":
                    if self.components[component]["used"] == False:
                        component_to_replace = str(component)

                        self.components[name] = self.components[component] 
                        del self.components[component]
        ...
        return (already_heated, hotplate_name, temp_to_return)
        
    def SetHotplate(self):
        pass
